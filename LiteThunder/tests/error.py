...
def to_cpp():
    ...
    print(
"""
#ifndef _CTP_ERROR_H_
#define _CTP_ERROR_H_
...
std::string get_error_message(int error){
    ...
    static std::map< int , std::string >  error_defs = {              
""")  
    
    for id,msg in error_defs.items():
        print('\t\t{ %s , "%s" },'%(id,msg))
    print("\t};")
    print("")
    print("\tauto itr = error_defs.find(error);")
    print('\tif(itr != error_defs.end()){ return "";}')
    print("\treturn itr->second;")
    print("}")
    
    print("#endif // _CTP_ERROR_H_")

to_cpp()


"""
python error.py > ../ctp-error.h
"""